...
local Skada    = LibStub("AceAddon-3.0"):NewAddon(addon, "Skada", "AceTimer-3.0")
_G.Skada       = Skada
...
-- =========================
-- | Display Skada Version |
-- =========================
local versionSkada = C_AddOns.GetAddOnMetadata("Skada", "Version")


if versionSkada ~= nil then

	print("|TInterface\\Icons\\Spell_Lightning_LightningBolt01:16|t" .. "\124cFF00FF00" .. "Skada Version: " .. versionSkada .. " (" .. locale .. ")")
end
				...
				args = {
					...
					rename = {
							...
							set	= function(win, val)
									...
									-- =========================================================
									local cwnHasName, cwnIsAllowedName = Skada:CheckWindowNames(val)

									if cwnIsAllowedName then

										local cwdHasName, cwdIsAllowedName = Skada:CheckWindowDuplicates(cwnHasName, "REN")

										if cwdIsAllowedName == true then
											...
											for position, entry in pairs(SkadaWindowsSort) do

												chunks = strsplittable("-", entry)

												if chunks[1] == oldname then

													newEntry = db.name .. "-" .. chunks[2]

													tinsert(SkadaWindowsSort, position, newEntry)
													tremove(SkadaWindowsSort, position + 1 , entry)

													tinsert(CurrentSkadaNames, position, db.name)
													tremove(CurrentSkadaNames, position + 1 , chunks[1])

													tinsert(SkadaWindowsChanges, date("%Y-%m-%d : %H-%M-%S") .. " : " .."REN" .. " : " ..
																     chunks[1] .. string.rep(" ", 22- string.len(chunks[1])) .. " : " ..
																     db.name  .. string.rep(" ", 22- string.len(db.name)))
													Skada:CurrentSort(SkadaWindowsChanges, "LDR")
												end
											end
											...
											Skada:CurrentTooltip("REN", oldname, db.name, true)
										else

											Skada:CurrentTooltip("REN", db.name, "", false)
										end
									end
								  end,
							order	= 1,
					...
					delete = {
							...
							func    = function()
									...
									-- =========================================================
									local newSkadaWindowsSort = {}

									Skada:tcopy(newSkadaWindowsSort, SkadaWindowsSort)
								  	Skada:DeleteWindow(db.name)

									tinsert(SkadaWindowsChanges, date("%Y-%m-%d : %H-%M-%S") .. " : " .. "DEL" .. " : " .. db.name  .. string.rep(" ", 22- string.len(db.name)))
									Skada:CurrentSort(SkadaWindowsChanges, "LDR")


									for position, entry in pairs(newSkadaWindowsSort) do

										delEntry = strsplit("-", entry)

										if delEntry == db.name then

											tremove(newSkadaWindowsSort, position, newSkadaWindowsSort[entry])
											tremove(CurrentSkadaNames, position, CurrentSkadaNames[delEntry])

											Skada:CurrentTooltip("DEL", db.name, "", true)
									...
									SkadaWindowsSort = {}
									Skada:tcopy(SkadaWindowsSort, newSkadaWindowsSort)
								  end,
						 },
				       }
			}
...
function Skada:CreateWindow(name, db, display)
	...
	-- =========================================================
	-- | "NEW" rebuilding SkadaWindowsSort/SkadaWindowsChanges |
	-- =========================================================
	local window   = Window:new()

	window.db      = db
	window.db.name = string.sub(string.upper(name), 1 ,1) .. string.sub(string.lower(name), 2)


	if isnew then

		tinsert(CurrentSkadaNames, window.db.name)
		tinsert(SkadaWindowsSort, window.db.name .. "-" .. "Alphabetical")

		tinsert(SkadaWindowsChanges, date("%Y-%m-%d : %H-%M-%S") .. " : " .. "NEW" .. " : " .. window.db.name  .. string.rep(" ", 22- string.len(window.db.name)))
		Skada:CurrentSort(SkadaWindowsChanges, "LDR")
	end
	...
	if self.displays[window.db.display] then
		...
		if isnew and find_mode(L["Damage"]) then
			...
			Skada:CurrentTooltip("DEF", eventWindowName_1, "", true)
	...
	if SkadaWindowsChanges == nil then

		SkadaWindowsChanges = {}

		tinsert(SkadaWindowsChanges, date("%Y-%m-%d : %H-%M-%S") .. " : " .. "NEW" .. " : " .. "Skada default" .. string.rep(" ", 22 - string.len("Skada default")))
	end
...
--	acd:SetDefaultSize("Skada", 800, 600)
	acd:SetDefaultSize("Skada", 830, 600)
...
-- ==================================
-- | validation: Skada-Window names |
-- ==================================
function Skada:CheckWindowNames(inputName)

	local outputIsAllowedName = true


	if string.len(inputName) == 0 or string.len(inputName) > 22 then

		outputHasName	    = ""
		outputIsAllowedName = false
	else

		for position = 1, string.len(inputName) do

			local character = string.sub(inputName, position, position)

			if not string.match(character, "[a-zA-Z]+") then
		...
		if outputIsAllowedName == true then outputHasName = string.sub(string.upper(inputName), 1 ,1) .. string.sub(string.lower(inputName), 2) end
	end

	return outputHasName, outputIsAllowedName
end
...
-- ==================================
-- | check: Skada-Window duplicates |
-- ==================================
function Skada:CheckWindowDuplicates(inputName, eventName)

	local hash	      = {}

	local res	      = {}
	local rCardSkadaNames = 0

	local uSkadaNames     = {}
	local uCardSkadaNames = 0


	for position, currentName in pairs(CurrentSkadaNames) do

		tinsert(uSkadaNames, currentName)
	end

	tinsert(uSkadaNames, inputName)
	uCardSkadaNames = #uSkadaNames


	for position, newCurrentName in pairs(uSkadaNames) do

		if (not hash[newCurrentName]) then

			res[#res+1]	     = newCurrentName
			hash[newCurrentName] = true
		end
	end
	...
	if uCardSkadaNames == rCardSkadaNames then

		outputHasName	    = inputName
		outputIsAllowedName = true
	else
		Skada:CurrentTooltip("INS", inputName, "", false)
		...
		if eventName == "INS" then
			...
			tinsert(SkadaWindowsChanges, date("%Y-%m-%d : %H-%M-%S") .. " : " .. "DUP" .. " : " .. inputName .. string.rep(" ", 22- string.len(inputName)) .. " : " .. "INS/DEL" .. string.rep(" ", 22- string.len("INS/DEL")))
			Skada:CurrentSort(SkadaWindowsChanges, "LDR")
		end


		if eventName == "REN" then

			tinsert(SkadaWindowsChanges, date("%Y-%m-%d : %H-%M-%S") .. " : " .. "DUP" .. " : " .. inputName .. string.rep(" ", 22- string.len(inputName)) .. " : " .. "REN/DEL" .. string.rep(" ", 22- string.len("REN/DEL")))
			Skada:CurrentSort(SkadaWindowsChanges, "LDR")
		end
	end
...
-- =======================================
	...
	if inputSort == "ALP" then
		...
		table.sort(inputTable,
				...
				function(a, b)
					...
					if a_Name == "Überheilung" then a_Name= "Uberheilung" end
					if b_Name == "Überheilung" then b_Name= "Uberheilung" end

 
					if string.sub(a_Name, 1, 29) < string.sub(b_Name, 1, 29) then
	...
	if inputSort == "CAT" then
		...
		for i, module in ipairs(inputTable) do table.insert(insTable, module:GetName()) end


		table.sort(insTable)


		for i, moduleName in ipairs(insTable) do table.insert(keyTable, moduleName) end
		...
		table.sort(inputTable,
				...
				function(a, b)
					...
					local a_score, b_score, a_sort_score, b_sort_score = 0, 0, 0, 0


					if locale == "deDE" then

						if a.category == L['Healing']	  then a_cat_score = 100 end
						if a.category == L['Power gains'] then a_cat_score = 200 end
						if a.category == L['Damage']	  then a_cat_score = 300 end
						if a.category == L['Other']	  then a_cat_score = 400 end

						if b.category == L['Healing']	  then b_cat_score = 100 end
						if b.category == L['Power gains'] then b_cat_score = 200 end
						if b.category == L['Damage']	  then b_cat_score = 300 end
						if b.category == L['Other']	  then b_cat_score = 400 end
					end


					if locale == "enUS" then

						if a.category == L['Damage']	  then a_cat_score = 100 end
						if a.category == L['Healing']	  then a_cat_score = 200 end
						if a.category == L['Power gains'] then a_cat_score = 300 end
						if a.category == L['Other']	  then a_cat_score = 400 end

						if b.category == L['Damage']	  then b_cat_score = 100 end
						if b.category == L['Healing']	  then b_cat_score = 200 end
						if b.category == L['Power gains'] then b_cat_score = 300 end
						if b.category == L['Other']	  then b_cat_score = 400 end
					end


					for i, keyName in pairs(keyTable) do

						if a:GetName() == keyName then
					...
					a_score = a_score + a_cat_score + a_sort_score


					for i, keyName in pairs(keyTable) do

						if b:GetName() == keyName then
					...
					b_score = b_score + b_cat_score + b_sort_score
	...
	-- ====================================
	-- | "CSA": channel sort alphabetical |
	-- ====================================
	if inputSort == "CSA" then
		...
		table.sort(inputTable,
				...
				function(a, b)
					...
					if locale == "deDE" then
						...
						if a == "whisper"		then a_score = a_score +  1 end		-- Flüstern
						if a == "target"		then a_score = a_score +  2 end		-- Flüsterziel
						if a == "guild"			then a_score = a_score +  3 end		-- Gilde
						if a == "party"			then a_score = a_score +  4 end		-- Gruppe
						if a == "instance_chat"		then a_score = a_score +  5 end		-- Instanz
						if a == "self"	 		then a_score = a_score +  6 end		-- Lokal
						if a == "officer"		then a_score = a_score +  7 end		-- Offizier
						if a == "say"			then a_score = a_score +  8 end		-- Sagen
						if a == "raid"			then a_score = a_score +  9 end		-- Schlachtzug

						for i, channel in pairs(csaKindTable) do				-- Kanal: 1 ...	Kanal: n ...

							if string.match(channel, a) then

								local offset = string.sub(channel, 8 , 8)
								a_score	     = a_score + 10 + 0.1 * tonumber(offset)
						...
						if b == "whisper"		then b_score = b_score +  1 end
						if b == "target"		then b_score = b_score +  2 end
						if b == "guild"			then b_score = b_score +  3 end
						if b == "party"			then b_score = b_score +  4 end
						if b == "instance_chat"		then b_score = b_score +  5 end
						if b == "self"	 		then b_score = b_score +  6 end
						if b == "officer"		then b_score = b_score +  7 end
						if b == "say"			then b_score = b_score +  8 end
						if b == "raid"			then b_score = b_score +  9 end

						for i, channel in pairs(csaKindTable) do

							if string.match(channel, b) then

								local offset = string.sub(channel, 8 , 8)
								b_score      = b_score + 10 + 0.1 * tonumber(offset)
					...
					if locale == "enUS" then
						...
						if a == "guild"			then a_score = a_score +   1 end
						if a == "instance_chat"		then a_score = a_score +   2 end
						if a == "officer"		then a_score = a_score +   3 end
						if a == "party"			then a_score = a_score +   4 end
						if a == "raid"			then a_score = a_score +   5 end
						if a == "say"	 		then a_score = a_score +   6 end
						if a == "self"			then a_score = a_score +   7 end
						if a == "whisper"		then a_score = a_score +   8 end
						if a == "target"		then a_score = a_score +   9 end

						for i, channel in pairs(csaKindTable) do
							if string.match(channel, a) then

								local offset = string.sub(channel, 10 , 10)
								a_score      = a_score + 10 + 0.1 * tonumber(offset)
						...
						if b == "guild"			then b_score = b_score +   1 end
						if b == "instance_chat"		then b_score = b_score +   2 end
						if b == "officer"		then b_score = b_score +   3 end
						if b == "party"			then b_score = b_score +   4 end
						if b == "raid"			then b_score = b_score +   5 end
						if b == "say"	 		then b_score = b_score +   6 end
						if b == "self"			then b_score = b_score +   7 end
						if b == "whisper"		then b_score = b_score +   8 end
						if b == "target"		then b_score = b_score +   9 end
		...
		table.sort(inputTable,
				...
				function(a, b)
					...
					if locale == "deDE" then
						...
						if a == "guild"			then a_score = a_score +  1 end		-- Gilde
						if a == "officer"		then a_score = a_score +  2 end		-- Offizier
						if a == "party"			then a_score = a_score +  3 end		-- Gruppe
						if a == "instance_chat"		then a_score = a_score +  4 end		-- Instanz
						if a == "raid"			then a_score = a_score +  5 end		-- Schlachtzug
						if a == "self"	 		then a_score = a_score +  6 end		-- Lokal
						if a == "say"			then a_score = a_score +  7 end		-- Sagen
						if a == "whisper"		then a_score = a_score +  8 end		-- Flüstern
						if a == "target"		then a_score = a_score +  9 end		-- Flüsterziel
						...
						if b == "guild"			then b_score = b_score +  1 end
						if b == "officer"		then b_score = b_score +  2 end
						if b == "party"			then b_score = b_score +  3 end
						if b == "instance_chat"		then b_score = b_score +  4 end
						if b == "raid"			then b_score = b_score +  5 end
						if b == "self"	 		then b_score = b_score +  6 end
						if b == "say"			then b_score = b_score +  7 end
						if b == "whisper"		then b_score = b_score +  8 end
						if b == "target"		then b_score = b_score +  9 end
					...
					if locale == "enUS" then
						...
						if a == "guild"			then a_score = a_score +  1 end
						if a == "officer"		then a_score = a_score +  2 end
						if a == "party"			then a_score = a_score +  3 end
						if a == "instance_chat"		then a_score = a_score +  4 end
						if a == "raid"			then a_score = a_score +  5 end
						if a == "self"	 		then a_score = a_score +  6 end
						if a == "say"			then a_score = a_score +  7 end
						if a == "whisper"		then a_score = a_score +  8 end
						if a == "target"		then a_score = a_score +  9 end
	...
	-- =========================================
	-- | "CSK": channel sort kind alphabetical |
	-- =========================================
	if inputSort == "CSK" then
	...
	-- ===========================
	-- | "LDR": log data reverse |
	-- ===========================
	if inputSort == "LDR" then

		table.sort(inputTable,

				function(a, b)

					if string.sub(a, 1, 21) > string.sub(b, 1, 21) then
...
-- =================================================
-- | gametootip: internal events "DEL"/"INS"/"REN" |
-- =================================================
function Skada:CurrentTooltip(eventName, eventWindowName_1, eventWindowName_2, eventResult)

	GameTooltip:SetOwner(UIParent, "ANCHOR_NONE")
	GameTooltip:SetPoint("CENTER", UIParent, "TOP",0, -100)
	GameTooltip:AddLine(L["Hint:"])
	...
	if eventName == "DEL" then
			...
			GameTooltip:AddLine("\124cFF00FF00" .. " > " .. eventWindowName_1 ..  " < " .. "\124r" .. L["Del"], 1, 1, 1)
	end
	...
	if eventName == "INS" then
		...
		if eventResult == true then
			...
			GameTooltip:AddLine(L["New Window"] .. "\124cFF00FF00" .. " > " .. eventWindowName_1 ..  " < " .. "\124r" .. L["has been created"], 1, 1, 1)
		else

			GameTooltip:AddLine("\124cFF00FF00" .. " > " .. eventWindowName_1 ..  " <   " .. "\124r" .. L["Duplicates insert"], 1, 1, 1)
		end
	end
	...
	if eventName == "REN" then
		...
		if eventResult == true then
			...
			if locale == "deDE" then
				...
				GameTooltip:AddLine("\124cFF00FF00" .. " > " .. eventWindowName_1 .. " < " .. "\124rwurde in" ..
						    "\124cFF00FF00" .. " > " .. eventWindowName_2 .. " < " .. "\124rumbenannt", 1, 1, 1)
			end


			if locale == "enUS" then

				GameTooltip:AddLine("\124cFF00FF00" .. " > " .. eventWindowName_1 .. " < " .. "\124rwas renamed to" ..
						    "\124cFF00FF00" .. " > " .. eventWindowName_2 .. " < ", 1, 1, 1)
			end
		else

			GameTooltip:AddLine("\124cFF00FF00" .. " > " .. eventWindowName_1 ..  " <   " .. "\124r" .. L["Duplicates rename"], 1, 1, 1)
		end
	end
