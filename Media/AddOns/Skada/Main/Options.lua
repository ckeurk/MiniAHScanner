...
function Skada:AddLoadableModuleCheckbox(mod, name, description)
	...
	-- | Ue -> Ü, german vowel corrupts the entry
	if name == "Ueberheilung" then name ="Überheilung" end
...

									{
										 ...
										 desc = L["Enter the name for the new window."],
										  set = function(self, val)
												...
												if cwnIsAllowedName == true then

													local cwdHasName, cwdIsAllowedName = Skada:CheckWindowDuplicates(cwnHasName, "INS")

													if cwdIsAllowedName == true then

														Skada:CreateWindow(cwdHasName, nil, newdisplay)
														Skada:CurrentTooltip("INS", cwdHasName, "", true)
													end
												end	
											end,
										order = 1,
